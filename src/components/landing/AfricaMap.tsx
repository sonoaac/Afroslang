...
const FLAG_COLORS: Record<string, string[]> = {
  DZA:['#006233','#FFFFFF'], AGO:['#CC0000','#000000','#FFCC00'],
  BEN:['#008751','#FCD116','#E8112D'], BWA:['#75AADB','#FFFFFF','#000000'],
  BFA:['#EF2B2D','#009A00'], BDI:['#CE1126','#FFFFFF','#1EB53A'],
  CPV:['#003893','#CF2027','#F7D116'], CMR:['#007A5E','#CE1126','#FCD116'],
  CAF:['#003082','#BC0026','#289728','#FFD100'], TCD:['#002664','#FECB00','#C60C30'],
  COM:['#3A75C4','#009A44','#FFFFFF'], COD:['#007FFF','#CE1126','#F7D618'],
  COG:['#009543','#FBDE4A','#DC241F'], DJI:['#6AB2E7','#12AD2B','#D7141A'],
  EGY:['#CE1126','#FFFFFF','#000000'], GNQ:['#3E9A00','#FFFFFF','#E32118'],
  ERI:['#4189DD','#D52B1E','#4CAF50'], SWZ:['#3E5EB9','#FCD116','#CE1126'],
  ETH:['#078930','#FCDD09','#DA121A'], GAB:['#009E60','#FCD116','#003189'],
  GMB:['#E75216','#0C1C8C','#3A7728'], GHA:['#CE1126','#FCD116','#006B3F'],
  GIN:['#CE1126','#FCD116','#009460'], GNB:['#CE1126','#FCD116','#009460'],
  CIV:['#F77F00','#FFFFFF','#009A44'], KEN:['#000000','#BB0000','#006600'],
  LSO:['#009543','#FFFFFF','#1B4F72'], LBR:['#BF0A30','#FFFFFF','#003478'],
  LBY:['#000000','#239E46','#D21034'], MDG:['#FFFFFF','#FC3D32','#007E3A'],
  MWI:['#000000','#CE1126','#339E35'], MLI:['#14B53A','#FCD116','#CE1126'],
  MRT:['#006233','#FFD700','#D21034'], MUS:['#EA2839','#1A206D','#EAC102','#00A551'],
  MAR:['#C1272D','#006233'], MOZ:['#009A44','#000000','#FCE100'],
  NAM:['#003580','#D21034','#009A44'], NER:['#E05206','#FFFFFF','#189B48'],
  NGA:['#008751','#FFFFFF'], RWA:['#20603D','#FAD201','#1F8ECD'],
  STP:['#12AD2B','#FFCD00','#D21034'], SEN:['#00853F','#FDEF42','#E31B23'],
  SYC:['#003F87','#FCD856','#D62828','#007A5C'], SLE:['#1EB53A','#FFFFFF','#0072C6'],
  SOM:['#4189DD','#FFFFFF'], ZAF:['#007A4D','#000000','#FFB612'],
  SSD:['#078930','#CE1126','#0F47AF','#FCD116'], SDN:['#D21034','#FFFFFF','#000000','#007229'],
  TZA:['#1EB53A','#000000','#00A3DD','#FCD116'], TGO:['#006A4E','#FFCE00','#D21034'],
  TUN:['#E70013','#FFFFFF'], UGA:['#000000','#FCDC04','#D90000'],
  ESH:['#000000','#FFFFFF','#007A3D'], ZMB:['#198A00','#DE2010','#EF7D00'],
  ZWE:['#006400','#FFD200','#D40000'],
};

const COUNTRY_LANGUAGES: Record<string, string[]> = {
  NG:['Hausa','Yoruba','Igbo'], ET:['Amharic'], SO:['Somali'], DJ:['Somali'],
  EG:['Arabic'], DZ:['Arabic','Berber'], MA:['Arabic','Berber'], TN:['Arabic'],
  LY:['Arabic'], SD:['Arabic'], MR:['Arabic'], TZ:['Swahili'], KE:['Swahili'],
  UG:['Swahili'], RW:['Swahili'], BI:['Swahili'], CD:['Swahili','Lingala'],
  ZA:['Zulu'], GH:['Twi'], BJ:['Yoruba'], TG:['Hausa'], NE:['Hausa'],
  SN:['Wolof'], GM:['Wolof'], BF:['Mòoré'], CG:['Lingala'], ZW:['Shona'],
  MW:['Chichewa'], ZM:['Chichewa'], SS:['Arabic'],
};
...
const COUNTRY_FACTS: Record<string, string> = {
  ...
  SN:'Nearly all Senegalese speak Wolof regardless of ethnicity.',
...
export function AfricaMap({ onCountrySelect, highlightedCodes, unlockedCodes }: AfricaMapProps) {
  ...
  const popupSetRef      = useRef<(d: PopupData | null) => void>(() => {});
  ...
  // ── Scroll-in animation (map always visible; class is additive) ───────────
  useEffect(() => {
    ...
    loadD3().then(async () => {
      ...
      features.forEach((feat: any) => {
        ...
        p.on('mousemove', function(event: MouseEvent) {
            ...
            tooltip.innerHTML     = `<strong>${name}</strong>${langs ? `<span style="display:block;font-size:10px;opacity:0.6;margin-top:2px">${langs.join(' · ')}</span>` : ''}`;
          })
        ...
        // Label: use ISO2 abbreviation (e.g. "NG", "KE")
        ...
        if (!isNaN(centroid[0]) && fs > 0) {
          svg.append('text')
            ...
            .style('font-family', "'Kalam', -apple-system, sans-serif")
  ...
  // ── Flag gradient for popup ───────────────────────────────────────────────
  function flagGradient(colors: string[]) {
    if (colors.length === 1) return `${colors[0]}80`;
    return `linear-gradient(135deg,${colors.map((c, i) => `${c}90 ${Math.round(i/(colors.length-1)*100)}%`).join(',')})`;
  }
  function flagTextColor(colors: string[]) {
  ...
  // ── Render ────────────────────────────────────────────────────────────────
  return (
    // Same single-div root as the original — no fragment, no <style> tag here
    <div ref={wrapRef} className={`lp-africa-map-wrap${mapIn ? ' am-map-entered' : ''}`}>
      <div ref={containerRef} className="lp-africa-map" />
      <div ref={tooltipRef}   className="lp-africa-map-tooltip" />

      {/* Mobile sticky-note popup — portal to body so no ancestor CSS affects it */}
      {popup && createPortal(
        <>
          {/* Backdrop */}
          <div
            ...
            style={{
              ...
              zIndex:9000,
              animation:'amBackdrop 0.25s ease',
            }}
          />
          ...
          <div style={{
            ...
            background: flagGradient(popup.colors),
            ...
            zIndex:9001,
            animation:'amPopupRise 0.38s cubic-bezier(0.34,1.56,0.64,1)',
            overflow:'hidden',
          }}>
            {/* Flag colour strip */}
            <div style={{display:'flex',height:5}}>
              {popup.colors.map((c,i) => <div key={i} style={{flex:1,background:c}} />)}
            </div>

            <div style={{padding:'clamp(1rem,5vw,1.5rem)'}}>
              {/* ISO badge + close */}
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.75rem'}}>
                <span className="am-tag" style={{background:'rgba(0,0,0,0.25)',color:'#fff',border:'1px solid rgba(255,255,255,0.22)'}}>
                  {popup.iso2}
                </span>
                <button
                  onClick={() => setPopup(null)}
                  style={{background:'rgba(0,0,0,0.22)',border:'1px solid rgba(255,255,255,0.2)',color:'#fff',
              ...
              {/* Country name */}
              <h2 style={{
                fontFamily:"'Kalam',sans-serif", fontWeight:700,
                fontSize:'clamp(1.5rem,7vw,2rem)', letterSpacing:'0.04em',
                textTransform:'uppercase', color:flagTextColor(popup.colors),
              ...
              {popup.languages.length > 0 && (
                <div style={{display:'flex',flexWrap:'wrap',gap:'0.4rem',marginBottom:'0.85rem'}}>
                  {popup.languages.map(lang => (
                    <span key={lang} className="am-tag"
              ...
              {popup.fact && (
                <p style={{
                  ...
                  color: flagTextColor(popup.colors) === '#ffffff' ? 'rgba(255,255,255,0.82)' : 'rgba(0,0,0,0.7)',
                ...
                {popup.languages.length > 0 && (
                  <button
                    ...
                    style={{
                      ...
                      color:'#fff', fontFamily:"'Kalam',sans-serif",
                      fontWeight:700, fontSize:'0.82rem', letterSpacing:'0.08em',
                      textTransform:'uppercase', padding:'0.7rem 1rem',
